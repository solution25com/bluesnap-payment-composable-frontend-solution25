...
import {BlueSnapConfigResponse, PfTokenResponse, TotalPriceResponse} from "../types/ConfigTypes.ts";
import {GooglePaymentResponse, GooglePaymentData} from "../types/GooglePayTypes.ts";
import {
...
axiosInstance.interceptors.request.use((config) => {
    ...
    if (!endpoint) {
        throw new Error('Shopware endpoint is not configured. Please set it before making API calls.');
    }
...
// BlueSnap API Object
const BlueSnapApi = {
    async processGooglePayment(googlePaymentData: GooglePaymentData): Promise<GooglePaymentResponse | void> {
        try {
    ...
    async fetchCart(): Promise<{ totalPrice: number; isoCountryCode: string }> {
        const response: AxiosResponse<TotalPriceResponse> = await axiosInstance.get('checkout/cart');

        // Extract the total price and country ISO code
        const totalPrice = response.data?.price?.totalPrice || 0; // Default to 0 if undefined
        const isoCountryCode =  (response.data?.deliveries?.[0]?.location?.country?.iso || ''); // Default to empty string if undefined
    ...
    async appleCreateWallet(body: AppleValidationBody): Promise<AppleCreateWalletResponse> {
        const response = await axiosInstance.post('bluesnap/apple-create-wallet', body);
    ...
    async vaultedShopper(body) {
        const response: AxiosResponse<VaultedShopperData> = await axiosInstance.post(
    ...
    async getVaultedShopperCreditCardData(vaultedShopperId: string): Promise<VaultedShopperData> {
    ...
    async BlueSnapCardCapture(body: CaptureRequestBody): Promise<{ data: CaptureResponse; vaultedShopperId: number | null }> {
        try {
    ...
    async getPfToken(): Promise<string | null> {
        try {
            const response: AxiosResponse<PfTokenResponse> = await axiosInstance.get(`bluesnap/get-pf-token`, {});
    ...
    async hostedCheckout(body: Record<string, any>): Promise<any> {
        try {
            ...
            const response: any = await axiosInstance.post('bluesnap/hosted-pages-link', body);

            console.log('Hosted Checkout Token Response:', response.data);
            return response.data;
        } catch (error: any) {
            console.error('Error in hostedCheckout:', error.response?.data || error.message);
            throw error;
        }
    },
